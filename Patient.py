...
class patient_vars:
    ...
    breast_adj_blood_test_atu_ac_review_probability_gen = (0 if random.random() < 0.3 else 1 for _ in
                                                                range(10000))
    ...
    breast_adj_blood_test_atu_t_review_probability_gen = (0 if random.random() < 0.3 else 1 for _ in
                                                               range(10000))
...
class CancerPatient:
    ...
    def __init__(self, env, id, condition, treatment_regime=None):
...
class BreastAdjuvant_Patient(CancerPatient):
    ...
    def __init__(self, env, id, treatment_regime):
        CancerPatient.__init__(self,env,id,condition=2,treatment_regime=treatment_regime)
        #
        #probability of ADR
            ...
            if(t_type=='admin'):
                ...
                if(t_resource_depend is not None):
                    if('pharmacist' in t_resource_depend):
                        ...
                        with hospital.pharmacists_resources.request() as pharmacists_req:
                            ...
                            print('patient:'+str(self.id)+' waiting for pharmacist')
                        ...
                        with hospital.cashier_resources.request() as cashiers_req:
                            # Increment count of number of patients waiting. 1 is subtracted
                            # from priority to align priority (1-3) with zero indexed list.
                            # if(self.id not in Global_vars.patients_db.keys()):
                            print('patient:'+str(self.id)+' waiting for cashier')
                            Global_vars.admin_time += t_time
                            Global_vars.patients_waiting += 1
                        ...
                        with hospital.nurse_resources.request(priority=self.priority) as nurse_req:
                            # Increment count of number of patients waiting. 1 is subtracted
                            # from priority to align priority (1-3) with zero indexed list.
                            # if(self.id not in Global_vars.patients_db.keys()):
                            print('patient:'+str(self.id)+' waiting for nurse')
                            Global_vars.admin_time += t_time
                            Global_vars.patients_waiting += 1
                    ...
                    if self.env.now >= Global_vars.warm_up:
                        Global_vars.patient_admin_queuing_results.loc[self.id] = admin_results
            if (t_type == 'treatment'):
                print('processing treatment time:' + t_desc + ' which takes:' + str(t_time) + ' mins')
                if (t_resource_depend is not None):
                    if ('chair' in t_resource_depend):
                        with hospital.chair_resources.request(priority=self.priority) as chair_req:
                            # print(Global_vars)
                            print('patient:' + str(self.id) + ' waiting for chair')
                            ...
                            if ('nurse' in t_resource_depend):
                                with hospital.nurse_resources.request(priority=self.priority) as nurse_req:
                                    ...
                                    Global_vars.patients_waiting += 1
                                    # print(Global_vars.patient_count)
                                    Global_vars.patients_waiting_for_nurses += 1
                                    Global_vars.patients_waiting_by_priority[self.priority - 1] += 1
                        ...
                        with hospital.doc_resources.request(priority=self.priority) as doc_req:
                            if (len(t_resource_depend) == 1):
                                # Increment count of number of patients waiting. 1 is subtracted
                                # from priority to align priority (1-3) with zero indexed list.
                                # if(self.id not in Global_vars.patients_db.keys()):
                                print('patient:' + str(self.id) + ' waiting for DMO')
                                Global_vars.treatment_time += t_time
                                Global_vars.patients_waiting += 1
                    ...
                    # del Patient.all_patients[self.id]
            if (t_type == 'time_between_visits'):
                print('patient:' + str(self.id) + ' waiting for in between visits for time='+str(t_time)+' mins')
                yield self.env.timeout(t_time)
        return
...
class BreastMetatstatic_Patient(CancerPatient):
    ...
    def __init__(self, env, id, treatment_regime):
        CancerPatient.__init__(self,env,id,condition=1,treatment_regime=treatment_regime)
        self.metastatic_regiment = random.randint(1, 3)
    def get_treatment_regime(self):
        return
